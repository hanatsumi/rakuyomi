...
function http.requestUnixSocket(socket_path, path, options)
  local executablePath = Paths.getPluginDirectory() .. "/uds_http_request"
  ...
  local output, err = io.popen('cat ' .. requestFilePath .. ' | RUST_LOG=trace RUST_BACKTRACE=1 ' .. executablePath, 'r')
  if output == nil then
